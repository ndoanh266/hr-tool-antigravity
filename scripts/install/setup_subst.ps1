...
:: 1. Start Google Drive if not running
...
:: 2. Wait up to 120 seconds for Google Drive and the target folder to be ready
set "targetDir="
set /a count=0
...
"@
            ...
            $content = @"
@echo off
rem Start Google Drive if not running
tasklist /FI "IMAGENAME eq GoogleDriveFS.exe" 2>NUL | find /I /N "GoogleDriveFS.exe">NUL
if "%ERRORLEVEL%" neq "0" (
    if exist "C:\Program Files\Google\Drive File Stream\launch.bat" (
        start "" "C:\Program Files\Google\Drive File Stream\launch.bat"
